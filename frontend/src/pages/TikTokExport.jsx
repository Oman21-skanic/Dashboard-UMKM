...
export default function TikTokExport() {
  ...
  const handleParseTemplate = async () => {
    ...
    try {
      ...
      data.sections.forEach((s) => {
        defaultOpen[s.id] = s.id === "basic" || s.id === "images";
      });
      ...
      setMessage({
        type: "error",
        text: err.response?.data?.msg || "Gagal membaca template.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (progress.filled < progress.total) {
        ...
        setMessage({
          type: "error",
          text: `Harap lengkapi semua field yang wajib diisi pada Produk yang ke-${i + 1}.`,
        });
        return;
      }
    }
    ...
    try {
      ...
      fd.append(
        "products",
        JSON.stringify(products.map((p) => ({ values: p }))),
      );
      ...
      const response = await fetch(
        `${API_URL}/api/tiktok-template/fill-and-export`,
        {
          method: "POST",
      ...
      link.setAttribute(
        "download",
        `tiktok_filled_${new Date().toISOString().slice(0, 10)}.xlsx`,
      );
      ...
      const savedCount =
        response.headers.get("x-saved-products") || products.length;
      setMessage({
        type: "success",
        text: `✅ File berhasil di-export! ${savedCount} produk juga tersimpan di inventori.`,
      });
      setStep(3);
    } catch (err) {
  ...
  const getProgress = (productIdx) => {
    ...
    const filled = required.filter((f) =>
      products[productIdx]?.[f.name]?.toString().trim(),
    ).length;
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a]">
      <div className="flex min-h-screen">
        {/* Sidebar */}
        <aside className="hidden md:flex w-[260px] flex-col bg-linear-to-b from-[#0b1e36] to-[#102e4a] text-white">
          <SidebarContent
        ...
        <div className="flex flex-1 flex-col min-w-0 bg-[#f8fafc]">
          {/* Mobile header */}
          <div className="md:hidden fixed top-0 left-0 right-0 z-30 flex items-center justify-between bg-linear-to-r from-[#0b1e36] to-[#102e4a] px-4 py-3 text-white">
          <Sheet>
            ...
            <SheetContent
              side="left"
              className="w-[280px] border-none bg-linear-to-b from-[#0b1e36] to-[#102e4a] p-0 text-white"
            >
              <SidebarContent
                displayName={displayName}
                initials={initials}
        ...
        <main className="flex-1 p-4 md:p-8 mt-14 md:mt-0 max-w-4xl mx-auto w-full">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#e2e8f0] bg-white text-[#64748b] hover:bg-[#f1f5f9] transition-all"
              >
                <ArrowLeft className="h-4 w-4" />
              </button>
              <div>
                <h1 className="text-xl font-bold text-[#0f172a]">
                  TikTok Template Filler
                </h1>
                <p className="text-xs text-[#94a3b8]">
                  Upload template → Isi form → Export siap import
                </p>
              </div>
            </div>
            {step > 1 && (
              <button
                onClick={resetAll}
                className="text-sm text-[#3182ce] font-semibold hover:underline"
              >
                ← Mulai Ulang
              </button>
            )}
          ...
          <div className="flex items-center gap-2 mb-6">
            {[
              ...
              <div key={s.num} className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-semibold hidden sm:inline ${step >= s.num ? "text-[#1e293b]" : "text-[#94a3b8]"}`}
                >
                  {s.label}
                </span>
                {i < 2 && (
                  <div
                    className={`w-8 h-0.5 ${step > s.num ? "bg-[#3182ce]" : "bg-[#e2e8f0]"}`}
                  />
                )}
              </div>
            ))}
          ...
          {message && (
            <div
              className={`px-4 py-3 rounded-xl text-sm font-medium mb-6 flex items-center justify-between ${
            ...
            <Card className="border-[#e2e8f0] shadow-sm">
              <CardHeader>
                <CardTitle className="text-base font-bold text-[#1e293b]">
                  Upload Template TikTok Shop
                </CardTitle>
                <p className="text-xs text-[#94a3b8] mt-1">
                  Download template dari TikTok Shop Seller Center, lalu upload
                  ke sini. Website akan membaca semua kolom & pilihan secara
                  otomatis.
                </p>
              </CardHeader>
              <CardContent>
                <label className="flex flex-col items-center justify-center px-6 py-12 border-2 border-dashed border-[#cbd5e1] rounded-2xl cursor-pointer hover:border-[#3182ce] hover:bg-[#eff6ff]/30 transition-all group">
                  ...
                  <p className="text-sm font-semibold text-[#1e293b]">
                    {templateFile
                      ? templateFile.name
                      : "Klik untuk pilih file template (.xlsx)"}
                  </p>
                  <p className="text-xs text-[#94a3b8] mt-1">
                    {templateFile
                      ? `${(templateFile.size / 1024).toFixed(1)} KB`
                      : "File template dari TikTok Shop Seller Center"}
                  </p>
                  <input
                    type="file"
                    accept=".xlsx,.xls"
                ...
                <Button
                  ...
                  className="w-full mt-4 bg-linear-to-r from-[#0b1e36] to-[#102e4a] hover:from-[#102e4a] hover:to-[#1a3f5c] text-white rounded-xl py-5 text-sm font-bold shadow-lg disabled:opacity-50"
                >
                  {loading ? (
                    <>
            ...
            <div className="space-y-5">
              ...
              {products.map((product, pIdx) => {
                ...
                return (
                  <Card
                    key={pIdx}
                    className="border-[#e2e8f0] shadow-sm overflow-hidden"
                  >
                    {/* Product header */}
                    <div className="flex items-center justify-between px-5 py-3 bg-linear-to-r from-[#f8fafc] to-white">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            <div className="w-24 h-1.5 bg-[#e2e8f0] rounded-full overflow-hidden">
                              <div
                                className="h-full bg-[#3182ce] rounded-full transition-all"
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                            <span className="text-[10px] text-[#94a3b8] font-semibold">
                              {progress.filled}/{progress.total} wajib
                            </span>
                          </div>
                        </div>
                      </div>
                      {products.length > 1 && (
                        <button
                          ...
                          className="flex h-8 w-8 items-center justify-center rounded-lg text-red-400 hover:bg-red-50 hover:text-red-600 transition-all"
                        >
                              ...
                              <div className="px-5 pb-4 space-y-3">
                                {sectionFields.map((field) => (
                                  <DynamicField
                                    ...
                                    onChange={(val) =>
                                      updateProduct(pIdx, field.name, val)
                                    }
                                  />
                                ))}
                              </div>
              ...
              <Button
                ...
                disabled={loading}
                className="w-full bg-linear-to-r from-[#0b1e36] to-[#102e4a] hover:from-[#102e4a] hover:to-[#1a3f5c] text-white rounded-xl py-6 text-sm font-bold shadow-lg disabled:opacity-50"
              >
                {loading ? (
                  <>
                  ...
                  <>
                    ...
                    Export {products.length} Produk ke Template TikTok
                  </>
                )}
              </Button>
            </div>
            ...
            <Card className="border-[#e2e8f0] shadow-sm">
              <CardContent className="flex flex-col items-center py-12">
                ...
                <p className="text-sm text-[#64748b] text-center max-w-md mb-2">
                  File XLSX sudah terdownload dan siap diimport ke TikTok Shop
                  Seller Center.
                </p>
                <p className="text-sm text-emerald-600 font-semibold text-center max-w-md mb-6">
                  📦 Produk juga sudah otomatis tersimpan di Inventori website
                  kamu!
                </p>
                <div className="flex gap-3">
                  <Button
                    onClick={resetAll}
                    variant="outline"
                    className="rounded-xl border-[#e2e8f0] px-6"
                  >
                    <Upload className="h-4 w-4 mr-2" />
                    Isi Template Baru
                  </Button>
                  <Button
                    onClick={() => navigate("/inventori")}
                    className="rounded-xl bg-[#3182ce] hover:bg-[#2b6cb0] text-white px-6"
                  >
...
function DynamicField({ field, value, onChange }) {
  ...
  const baseInputClass =
    "w-full border rounded-lg px-3 py-2 text-sm outline-none transition-all focus:ring-2 focus:ring-[#3182ce]/30";
  const borderClass =
    field.isRequired && !value?.toString().trim()
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        ...
        {field.isConditional && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-amber-100 text-amber-600 font-bold">
            Sesuai Syarat
          </span>
        )}
        {field.isOptional && (
      ...
      {/* Description tooltip */}
      {field.description && (
        <p className="text-[10px] text-[#94a3b8] mb-1 leading-snug line-clamp-2">
          {field.description}
        </p>
      )}
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="relative shrink-0">
            {isUploading ? (
              ...
              <label
                className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg border border-[#e2e8f0] bg-white hover:bg-[#f8fafc] hover:border-[#3182ce] transition-all"
