...
// ── Kirim OTP via Brevo API (HTTP) / Nodemailer Fallback ──────────────────────
async function sendOTPEmail(toEmail, otp) {
  ...
  if (brevoKey) {
    try {
      ...
      return; // Sukses via Brevo
    } catch (err) {
      console.error('[Brevo Error]', err.response?.data || err.message);
      brevoFailed = true;
      // Jika Brevo gagal, jangan langsung throw, kita coba fallback Nodemailer
    }
  }

  // Fallback ke Nodemailer menggunakan Gmail
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    if (brevoFailed) {
      throw new Error('Gagal mengirim email lewat Brevo dan tidak ada konfigurasi Gmail untuk fallback.');
    }
    throw new Error('Konfigurasi Email (Brevo / Gmail) belum diatur.');
  }
  ...
  try {
    ...
    console.log('[Nodemailer] OTP berhasil dikirim via Gmail.');
  } catch (err) {
    console.error('[Nodemailer Error]', err.message);
    throw new Error('Gagal mengirim email OTP lewat Brevo maupun Nodemailer.');
  }
}
